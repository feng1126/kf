/*
 * @Author: your name
 * @Date: 2021-04-21 16:07:35
 * @LastEditTime: 2021-05-18 16:02:35
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \code\src\tools\UTMTran.cpp
 */
...
namespace tool_t
{
    ...
    static std::shared_ptr<UTMTransform> m_instance;
    std::shared_ptr<UTMTransform> UTMTransform::instance(void)
    {
        if (m_instance.get() == 0)
        {
            m_instance.reset(new UTMTransform);
        }
    ...
    char UTMTransform::UTMLetterDesignator(double Lat)
    {
    ...
    double UTMTransform::LLToUTM(const double Lat, const double Long, double& UTMEasting, double& UTMNorthing)
    {
        //std::string UTMZone;
        double a = WGS84_A;
        ...
        double converge_angle = sin(LatRad) * (LongTemp - LongOrigin);		//子午线收敛角，用来补偿航向角
        return converge_angle;
    }

    void UTMTransform::UTMtoLL(const double UTMNorthing, const double UTMEasting,
