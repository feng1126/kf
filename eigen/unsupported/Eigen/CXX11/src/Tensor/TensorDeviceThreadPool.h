...
class Barrier {
 public:
  Barrier(unsigned int count) : mState(count << 1), notified_(false) {
    eigen_assert(((count << 1) >> 1) == count);
  }
  ~Barrier() {
    eigen_assert((mState>>1) == 0);
  }

  void Notify() {
    unsigned int v = mState.fetch_sub(2, std::memory_order_acq_rel) - 2;
    if (v != 1) {
  ...
  void Wait() {
    unsigned int v = mState.fetch_or(1, std::memory_order_acq_rel);
    if ((v >> 1) == 0) return;
    std::unique_lock<std::mutex> l(mu_);
    while (!notified_) {
      cv_.wait(l);
 ...
 private:
  ...
  std::atomic<unsigned int> mState;  // low bit is waiter flag
  bool notified_;
};
