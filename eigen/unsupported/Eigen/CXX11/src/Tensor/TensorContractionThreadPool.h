...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType>, ThreadPoolDevice> :
          ...
          nn0_(nn0)
  {
        ...
        mStateswitch_[x] =
            x == 0
                ...
                : (parallel_pack_ ? nn_ + nm_ : (shard_by_col_ ? nn_ : nm_)) +
                      (x == P - 1 ? nm_ * nn_ : 0);
        mStatepacking_ready_[x] =
            parallel_pack_ ? 0 : (shard_by_col_ ? nm_ : nn_);
        mStatekernel_[x] = new std::atomic<uint8_t>*[nm_];
        for (Index m = 0; m < nm_; m++) {
          mStatekernel_[x][m] = new std::atomic<uint8_t>[nn_];
    ...
    ~Context() {
      for (Index x = 0; x < P; x++) {
        for (Index m = 0; m < nm_; m++) delete[] mStatekernel_[x][m];
        delete[] mStatekernel_[x];
      }
   ...
   private:
    ...
    //
    // mStateswitch_ tracks when we are ready to switch to the next k slice.
    // mStatekernel_[m][n] tracks when we are ready to kick off kernel (m, n).
    ...
    std::atomic<uint8_t>** mStatekernel_[P];
    // mStateswitch_ is frequently modified by worker threads, while other
    // fields are read-only after constructor. Let's move it to a separate cache
    // line to reduce cache-coherency traffic.
    char pad_[128];
    std::atomic<Index> mStatepacking_ready_[P];
    std::atomic<Index> mStateswitch_[P];
    ...
    void signal_packing(Index k) {
      ...
      Index s = mStatepacking_ready_[k % P].fetch_sub(1);
      eigen_assert(s > 0);
      if (s != 1) return;
      mStatepacking_ready_[k % P] = shard_by_col_ ? nm_ : nn_;
      enqueue_packing(k, shard_by_col_);
    }

    void signal_kernel(Index m, Index n, Index k, bool sync) {
      std::atomic<uint8_t>* state = &mStatekernel_[k % P][m][n];
      Index s = state->load();
      eigen_assert(s > 0);
    ...
    void signal_switch(Index k, Index v = 1) {
      Index s = mStateswitch_[k % P].fetch_sub(v);
      eigen_assert(s >= v);
      if (s != v) return;
