...
//
// Algorihtm outline:
// There are two main variables: predicate (managed by user) and mState.
// Operation closely resembles Dekker mutual algorithm:
// https://en.wikipedia.org/wiki/Dekker%27s_algorithm
// Waiting thread sets mState then checks predicate, Notifying thread sets
// predicate then checks mState. Due to seq_cst fences in between these
// operations it is guaranteed than either waiter will see predicate change
// and won't block, or notifying thread will see mState change and will unblock
...
class EventCount {
 public:
  ...
  EventCount(MaxSizeVector<Waiter>& waiters) : waiters_(waiters) {
    ...
    mState = kStackMask | (kEpochMask - kEpochInc * waiters.size() * 2);
  }

  ~EventCount() {
    // Ensure there are no waiters.
    eigen_assert((mState.load() & (kStackMask | kWaiterMask)) == kStackMask);
  }
  ...
  void Prewait(Waiter* w) {
    w->epoch = mState.fetch_add(kWaiterInc, std::memory_order_relaxed);
  ...
  void CommitWait(Waiter* w) {
    ...
    for (;;) {
      if (int64_t((state & kEpochMask) - epoch) < 0) {
        ...
        EIGEN_THREAD_YIELD();
        state = mState.load(std::memory_order_seq_cst);
        continue;
      }
  ...
  void CancelWait(Waiter* w) {
    ...
    for (;;) {
      if (int64_t((state & kEpochMask) - epoch) < 0) {
        ...
        // calls either CancelWait or CommitWait, or is notified.
        EIGEN_THREAD_YIELD();
        state = mState.load(std::memory_order_relaxed);
        continue;
      }
      // We've already been notified.
      if (int64_t((state & kEpochMask) - epoch) > 0) return;
      ...
      eigen_assert((state & kWaiterMask) != 0);
      if (mState.compare_exchange_weak(state, state - kWaiterInc + kEpochInc,
